...
def rotate_half(x):
    # Build the [-x2, x1, -x4, x3, ...] tensor of the sin part of the positional encoding
    x1 = x[..., : x.shape[-1] // 2]
    x2 = x[..., x.shape[-1] // 2 :]
...
class GemmaRotaryEmbedding(nn.Module):
    ...
    def forward(self, x, position_ids, seq_len=None):
        ...
        with torch.autocast(device_type=device_type, enabled=False):
            # Multyply each theta by the position (which is the argument of the sin and cos functions)
...
class GemmaAttention(nn.Module):
    ...
    def forward(
        ...
        cos, sin = self.rotary_emb(query_states, position_ids, seq_len=None)
        ...
        # Multiply by W.o [Batch_size, Seq_len_Q, Hidden_size]
